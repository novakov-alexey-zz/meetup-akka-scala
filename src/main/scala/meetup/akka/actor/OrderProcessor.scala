...
class OrderProcessor(orderDao: IOrderDao) extends PersistentActor with AtLeastOnceDelivery {
  ...
  val nrOfInstances = 5
  val orderLogger = context.actorOf(RoundRobinPool(nrOfInstances).props(Props(classOf[OrderLogger], orderDao)), "orderLogger").path
  ...
  private def generateOrderId(event: Any) {
    event match {
      case newOrder: NewOrder =>
        orderIdGenerator ! newOrder.order
      case m => unhandled(m)
    }
  }
